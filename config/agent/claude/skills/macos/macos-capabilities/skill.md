...
| Split a service "to use fewer resources" | Usually wrong — tune in-process first | process-separation.md |
| Crash-isolate / sandbox-harden a service | XPC Service (decision framework) | process-separation.md |
...
5. **Process Separation**: `process-separation.md`
   - When to split into a process vs. keep in-process
   - The resource myth (splitting raises memory, not lowers it)
   - XPC ephemeral-lifecycle gotcha for long-lived connections
   - Inside-out notarization cost
